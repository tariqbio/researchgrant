...
class Grant(Base):
    ...
    # Two FKs to users — must name foreign_keys on EVERY relationship touching users
    ...
    # ── Relationships — foreign_keys explicit on BOTH user FK paths ───────────
    source = relationship("Source", back_populates="grants")
    org_publisher = relationship(
        "User",
        foreign_keys=[org_publisher_id],
        back_populates="published_grants",
    )
