...
const STATUS_LABELS: Record<string, { label: string; color: string; done: boolean }> = {
  pending_ocr: { label: 'Queued', color: 'text-gray-400', done: false },
  ocr_running: { label: 'Reading document...', color: 'text-blue-600', done: false },
  ocr_failed: { label: 'Text extraction failed', color: 'text-red-600', done: true },
  ai_running: { label: 'Extracting fields...', color: 'text-purple-600', done: false },
...
function JobStatusRow({ job }: { job: IngestionJob }) {
  ...
  const filename = job.raw_file_path?.split(/[\\/]/).pop() ?? job.raw_url ?? 'Uploaded notice'
      ...
      <div className="flex items-center gap-3 min-w-0">
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs text-gray-400">
            ...
            {job.ocr_confidence != null && ` - ${(job.ocr_confidence * 100).toFixed(0)}%`}
          </p>
        </div>
      </div>
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    for (const file of Array.from(files)) {
      if (!/\.(pdf|docx|txt)$/i.test(file.name)) {
        setError(`"${file.name}" is not supported. Upload PDF, DOCX, or TXT files.`)
        continue
      }
      try {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6">
      <div className="flex items-center justify-between mb-5">
        <div>
          <h1 className="text-lg font-medium text-gray-900">Upload Notice</h1>
          <p className="text-sm text-gray-400 mt-0.5">
            Upload a grant notice PDF, DOCX, or TXT. Extraction runs automatically.
          </p>
        </div>
      </div>
      ...
      <div
        ...
        className={`border-2 border-dashed rounded-xl p-10 text-center cursor-pointer transition-colors ${
          dragging ? 'border-emerald-400 bg-emerald-50' : 'border-gray-200 bg-white hover:border-gray-300 hover:bg-gray-50'
        }`}
      >
        <input
          ref={fileInputRef}
        ...
        <p className="text-sm text-gray-500 mb-1">
          {dragging ? 'Drop files here' : 'Drag and drop notice files here'}
        </p>
        <p className="text-xs text-gray-400">PDF, DOCX, TXT - max 20MB per file</p>
        {upload.isLoading && <p className="text-xs text-blue-500 mt-3 animate-pulse">Uploading...</p>}
      </div>
      ...
      <div className="mt-4 bg-gray-50 rounded-xl p-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ['Text', 'PDF/DOCX text extraction; OCR fallback when configured'],
            ['AI extract', 'Claude when configured; safe fallback otherwise'],
            ['Review', 'Draft grant appears in review queue'],
            ['Publish', 'Approve and notify matching researchers'],
          ].map(([label, desc]) => (
            <div key={label} className="bg-white border border-gray-100 rounded-lg p-3">
              <p className="text-xs font-medium text-gray-700">{label}</p>
              <p className="text-[11px] text-gray-400 leading-snug mt-1">{desc}</p>
            </div>
          ))}
        </div>
      </div>
