...
export default function OrgDashboard() {
  ...
  useEffect(() => { orgApi.myGrants().then(r => setGrants(r.data)).catch(() => {}); }, []);
  ...
  if (user?.account_status === 'pending') return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow p-10 max-w-md text-center">
        ...
        <p className="text-gray-500 text-sm">Your organization account is under review. You'll receive an email once verified — usually within 1-2 business days.</p>
        <p className="mt-4 text-sm text-gray-400">Registered as: <strong>{user.org_name}</strong></p>
      </div>
    </div>
  );

  const published = grants.filter(g => g.status === 'published');
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-5xl mx-auto px-4 py-8">
        <div className="mb-6">
          ...
          <p className="text-gray-500 text-sm">Manage grant calls and review applications</p>
        </div>
        <div className="grid grid-cols-3 gap-4 mb-6">
          {[['Published', published.length, 'green'], ['Total', grants.length, 'blue'], ['Expired', grants.filter(g=>g.status==='expired').length, 'gray']].map(([l,v,c]:any) => (
        ...
        <div className="flex gap-3 mb-6">
          <Link to="/org/publish" className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-5 py-2.5 rounded-lg text-sm transition">+ Publish New Grant Call</Link>
        </div>
        <div className="bg-white rounded-2xl shadow-sm border p-6">
          ...
          {grants.length === 0 ? (
            <div className="text-center py-10 text-gray-400">
              ...
              <p className="text-sm">No grants published yet.</p>
              <Link to="/org/publish" className="text-blue-600 text-sm font-medium mt-2 inline-block">Publish your first grant →</Link>
            </div>
          ) : grants.map(g => (
            <div key={g.id} className="flex items-center justify-between p-4 border rounded-xl hover:bg-gray-50 mb-2">
              ...
              <div className="flex items-center gap-3">
                ...
                <Link to={`/org/grants/${g.id}/applications`} className="text-blue-600 text-xs font-medium hover:underline">Applications →</Link>
              </div>
            </div>
          ))}
        </div>
